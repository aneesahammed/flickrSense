...
namespace Microsoft.Toolkit.Uwp.Services
{
    ...
    public abstract class DataProviderBase<TConfig>
    {
        ...
        public async Task<IEnumerable<TSchema>> LoadDataAsync<TSchema>(TConfig config,int pageIndex, int maxRecords, IParser<TSchema> parser)
            where TSchema : SchemaBase
        {
            ...
            var result = await GetDataAsync(config,pageIndex,maxRecords, parser);
            if (result != null)
            {
                return result
                    .Take(maxRecords)
        ...
        protected abstract Task<IEnumerable<TSchema>> GetDataAsync<TSchema>(TConfig config,int pageIndex,int maxRecords, IParser<TSchema> parser)
            where TSchema : SchemaBase;
